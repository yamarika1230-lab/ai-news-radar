    ...
    <div>
      ...
      <div className="space-y-1.5">
        {keywords.map(({ keyword, change, hot, searchUrl, relatedKeywords }) => {
          // change はスコア（数値文字列）またはパーセント表記
          const score = parseInt(change, 10);
          const isNumericScore = !isNaN(score) && !change.includes("%");
              ...
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-xs font-semibold ${
                    hot ? "text-[#E07050]" : "text-[#A0A09C]"
                  }`}
                >
                  {isNumericScore ? score : change}
                </span>
              </div>
              {relatedKeywords && relatedKeywords.length > 0 && (
                <div className="mt-1 flex flex-wrap gap-x-2 gap-y-0.5 pl-5">
