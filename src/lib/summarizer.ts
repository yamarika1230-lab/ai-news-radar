...
// ---------------------------------------------------------------------------
// メインエクスポート: extractTrendingKeywords（記事からClaude APIで生成）
// ---------------------------------------------------------------------------
  ...
  try {
    const message = await client.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: `以下はAI/テクノロジー関連のニュース記事タイトル一覧です。
これらの記事から、今日のAI/テクノロジー分野のトレンドキーワードを最大10個抽出してください。

ルール:
- AI、テクノロジー、ITに直接関連するキーワードのみ（天気、スポーツ、政治等は絶対に含めない）
- 日本語で出力（英語の固有名詞・略語はそのまま可。例: RAG, Claude Code, MCP）
- 各キーワードに注目度スコア（1-100）を付与
- hot: スコア70以上は true
- 各キーワードにAI/テクノロジー文脈の関連キーワードを5つ付与

記事タイトル一覧:
${titles}

JSON配列で返してください。コードブロックで囲まないでください:
[{"keyword":"キーワード","score":85,"hot":true,"relatedKeywords":["関連語1","関連語2","関連語3","関連語4","関連語5"]}]`,
        },
      ],
    });
    ...
    const parsed = parseClaudeResponse(block.text.trim()) as {
      keyword?: string;
      score?: number;
      hot?: boolean;
    ...
    const results: TrendingKeyword[] = parsed
      .filter((k) => typeof k.keyword === "string" && k.keyword.length > 0)
      .slice(0, 10)
      .map((k) => {
        ...
        return {
          ...
          change: `${score}`,
          hot: k.hot ?? score >= 70,
          searchUrl: `https://www.google.com/search?q=${encodeURIComponent(k.keyword!)}`,
          relatedKeywords: (k.relatedKeywords ?? []).slice(0, 5).map((r) => ({
            keyword: String(r),
            searchUrl: `https://www.google.com/search?q=${encodeURIComponent(String(r))}`,
          })),
        };
    ...
    // スコア降順ソート
    results.sort((a, b) => Number(b.change) - Number(a.change));

    console.log(
      `[Summarizer] トレンドキーワード生成: ${results.length}件`,
    );
    return results;
  } catch (error) {
